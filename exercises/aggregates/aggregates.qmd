...
Every player to play in the NBA this season is shown, alongside their stats.
...
The order of the total players per position is SG, C, SF, PF, PG.
...
Players that get traded and are considered for two positions (Ex: PG-SG).
...
**ANSWER** The most interesting to me is that power forwards average the most minutes per game. I would think that most teams would want their point guard -- since that's the supposed "conuctor of an offense" to handle the most minutes on average. Instead, it's power forwards, a position that typically needs rest -- not as much as center -- but still a good amount considering bigger players play that position. A possible story here could be to go further with this data and find out which teams provide the most minutes for specific positions, and see if there's a correlation with success.